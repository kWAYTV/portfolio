...
interface SplitEditorViewProps {
  ...
  closeOtherTabs: (pathname: string, groupIndex: number, href: string) => void;
...
export function SplitEditorView({
  ...
  return (
    <div className="hidden min-h-0 min-w-0 flex-1 md:flex" ref={containerRef}>
      {editorGroups.map((group, i) => (
        <React.Fragment key={`group-${i}-${group.tabs.join("-")}`}>
          <div
          ...
          >
            <EditorPane
              ...
              onCloseOtherTabs={(gi, href) =>
                closeOtherTabs(pathname, gi, href)
              }
              onCloseTab={(gi, href) => closeTab(pathname, gi, href)}
